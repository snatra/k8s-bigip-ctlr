...
func main() {
	...
	if err = appMgr.AgentCIS.Init(getAgentParams(*agent)); err != nil {
		log.Fatalf("[INIT] unable to initialize agent %v error: err: %+v\n", *agent, err)
		os.Exit(1)
	}
	...
	if *agent == cisAgent.AS3Agent {
		...
		if err = appMgr.AgentCCCL.Init(getAgentParams(cisAgent.CCCLAgent)); err != nil {
			log.Fatalf("[INIT] unable to initialize L2L3 Agent %v error: err: %+v\n", *agent, err)
			os.Exit(1)
		}
...
func cleanupOtherAgents(exclAgent, partition string) error {
	...
	agentList = append(agentList, cisAgent.AS3Agent) // thiscan include cisAgent.CCCLAgent etc.
...
func getProcessAgentLabelFunc() func(map[string]string) bool {
	switch *agent {
	case cisAgent.AS3Agent:
		return func(m map[string]string) bool {
			if m["overrideAS3"] == "true" || m["as3"] == "true"{
				return true
			}
			return false
		}
