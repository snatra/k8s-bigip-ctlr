...
func (appMgr *Manager) agentResponseWorker() {
	...
	for rspMsg := range appMgr.agRspChan {
		if rspMsg.(resource.ResourceResponse).AdmitStatus {
			appMgr.deployARP(rspMsg.(resource.ResourceResponse).Members)
		...
		if rspMsg.(resource.ResourceResponse).FdbRecords {
			appMgr.deployFDB()
		}
	}
}
