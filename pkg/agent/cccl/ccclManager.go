...
type CCCLManager struct {
	...
	httpAddress       string
	oldFDBRecords     map[string]*BigIPConfig
	ResourceRequest
	GlobalSection
	BigIPSection
}
...
func NewCCCLManager(params *Params) *CCCLManager {
	CCCLManager := CCCLManager{
		...
		flannelName:       params.FlannelName,
		httpAddress:       params.HttpAddress,
		nodePollInterval:  params.NodePollInterval,
	}
	return &CCCLManager
}
...
func getConfigWriter() writer.Writer {
	...
	if nil != err {
		log.Fatalf("[CCCL] Failed creating ConfigWriter tool: %v", err)
		os.Exit(1)
	}
	return configWriter
}
...
func (c *CCCLManager) SetupL2L3() {
	...
	if nil != err {
		log.Fatalf("[CCCL] Could not initialize subprocess configuration: %v", err)
	}
	c.subPid = <-subPidCh
	...
	if nil != err {
		log.Fatalf("[CCCL] Required polling utility for node updates failed setup: %v", err)
		os.Exit(1)
	}
	c.NodePoller.Run()
}
...
func (c *CCCLManager) setupNodePolling() error {
	...
	if nil != err {
		return fmt.Errorf("[CCCL] error registering node update listener: %v",
			err)
	}
	...
	if 0 != len(c.vxlanMode) {
		...
		if nil != err {
			return fmt.Errorf("[CCCL] error creating vxlan manager: %v", err)
		}

		// Register vxMgr to watch for node updates to process fdb records
		err = c.NodePoller.RegisterListener(vxMgr.ProcessNodeUpdate)
		if nil != err {
			return fmt.Errorf("[CCCL] error registering node update listener for vxlan mode: %v",
				err)
		}
		if c.eventChan != nil {
...
func (c *CCCLManager) CleanPythonDriver() {
	if 0 != c.subPid {
		var proc *os.Process
		proc, err := os.FindProcess(c.subPid)
		if nil != err {
			log.Warningf("[CCCL] Failed to find sub-process on exit: %v", err)
		}
		err = proc.Signal(os.Interrupt)
		if nil != err {
			log.Warningf("[CCCL] Could not stop sub-process on exit: %d - %v", c.subPid, err)
		}
	}
}
