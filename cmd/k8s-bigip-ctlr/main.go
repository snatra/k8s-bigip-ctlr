...
func main() {
	...
	log.Infof("Creating Agent for %v", appMgr.Agent)
	appMgr.AgentCIS, err = cisAgent.CreateAgent(appMgr.Agent)
	if err != nil {
		log.Fatalf("unable to create agent %v error: err: %+v\n", appMgr.Agent, err)
		os.Exit(1)
	}
	err = cleanupOtherAgents(appMgr.Agent, resource.DEFAULT_PARTITION)
	if err != nil {
		os.Exit(1)
	}
	log.Infof("Initializing CIS configured Agent %v", appMgr.Agent)
	...
	if appMgr.Agent == cisAgent.AS3Agent {
		...
		if err = appMgr.AgentCCCL.Init(getAgentParams(cisAgent.CCCLAgent)); err != nil {
			log.Fatalf("unable to initialize L2L3 Agent %v error: err: %+v\n", appMgr.Agent, err)
			os.Exit(1)
		}
...
func cleanupOtherAgents(exclAgent, partition string) error {
	...
	agentList = append(agentList, cisAgent.AS3Agent) // TODO, cisAgent.CCCLAgent)
	for _, agent := range agentList {
		log.Infof("Cleaning up Agent %v", agent)
		if exclAgent != agent {
			agentCIS, err := cisAgent.CreateAgent(agent)
			if err != nil {
				log.Fatalf("Failed to create agent: %v", agent)
				return err
			}
...
func getAppManagerParams() appmanager.Params {
	return appmanager.Params{
		// ConfigWriter:           getConfigWriter(), // TODO This will go into CCCL Manager
