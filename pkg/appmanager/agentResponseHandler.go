...
func (appMgr *Manager) eraseRouteAdmitStatus(route routeapi.Route) {
	for i, _ := range route.Status.Ingress {
		if route.Status.Ingress[i].RouterName == F5RouterName {
			...
			if err != nil {
				log.Errorf("Error while Erasing Route Admit Status: %v\n", err)
			} else {
				log.Debugf("Admit Status Erased for Route - %v\n", route.ObjectMeta.Name)
			}
			return
		}
	}
...
func (appMgr *Manager) agentResponseWorker() {
	log.Debugf("Agent Response Worker started and blocked on channel  %v", appMgr.agRspChan)
	for rspMsg := range appMgr.agRspChan {
		if rspMsg.(resource.MessageResponse).AdmitStatus {
			appMgr.deployARP()
			log.Debugf("Sending ARP entries")
			if appMgr.routeClientV1 != nil {
				log.Debugf("Updating Route Admit Status")
