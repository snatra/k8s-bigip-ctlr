...
func (vxm *VxlanMgr) ProcessNodeUpdate(obj interface{}, err error) {
	if nil != err {
		log.Warningf("[CCCL] Vxlan manager (%s) unable to get list of nodes: %v",
			vxm.vxLAN, err)
		return
	}
	...
	if false == ok {
		log.Warningf("[CCCL] Vxlan manager (%s) received poll update with unexpected type",
			vxm.vxLAN)
		return
	}
	...
	if nil != err {
		log.Warningf("[CCCL] Vxlan manager (%s) failed to write fdb config section: %v",
			vxm.vxLAN, err)
	} else {
		select {
		case <-doneCh:
			log.Debugf("[CCCL] Vxlan manager (%s) wrote config section: %v",
				vxm.vxLAN, records)
		case e := <-errCh:
			log.Warningf("[CCCL] Vxlan manager (%s) failed to write config section: %v",
				vxm.vxLAN, e)
		case <-time.After(time.Second):
			log.Warningf("[CCCL] Vxlan manager (%s) did not receive write response in 1s",
				vxm.vxLAN)
		}
	}
}
...
func ipv4ToMac(addr string) string {
	...
	if len(ip) != 4 {
		log.Errorf("[CCCL] Bad IPv4 address format specified for FDB record: %s", addr)
		return ""
	}
	var intIP [4]int
	for i, val := range ip {
...
func (vxm *VxlanMgr) ProcessAppmanagerEvents(kubeClient kubernetes.Interface) {
	go func() {
		log.Debugf("[CCCL] Vxlan Manager waiting for pod events from appManager.")
		for {
			select {
			case pods := <-vxm.podChan:
				if pods, ok := pods.([]resource.Member); ok {
					...
					log.Errorf("[CCCL] Vxlan Manager could not read Endpoints from appManager channel.")
				}
			}
		}
	}()
...
func (vxm *VxlanMgr) addArpForPods(pods interface{}, kubeClient kubernetes.Interface) {
	...
	if nil != err {
		log.Errorf("[CCCL] Vxlan Manager could not list Kubernetes Pods for ARP entries: %v", err)
		return
	}
	kubeNodes, err := kubeClient.CoreV1().Nodes().List(metav1.ListOptions{})
	if nil != err {
		log.Errorf("[CCCL] Vxlan Manager could not list Kubernetes Nodes for ARP entries: %v", err)
		return
	}
	...
	if nil != err {
		log.Warningf("[CCCL] Vxlan manager (%s) failed to write arp config section: %v",
			vxm.vxLAN, err)
	} else {
		select {
		case <-doneCh:
